...
import { getPermissions } from "@saas/auth";
...
export const agentRouter = new Elysia({ name: "agent-router" })
  .use(contextPlugin)
  .ws("/agent", {
    async upgrade(ctx: any) {
      const { session, user, db, set } = ctx;
    ...
    close(ws: any) {
      (ws.data as SessionData).agent.approvalChannel.cleanup();
    },
  });
...
const buildSystemPrompt = (ctx: SessionData): string => {
