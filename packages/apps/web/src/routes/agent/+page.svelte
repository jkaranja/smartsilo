<script lang="ts">
  ...
  import { publicWebConfig } from '$lib/client/config';
  ...
  const navTopics = $derived<NavItem[]>(
    ws.topics.map(t => ({
      ...
      label: t.label,
      icon:  t.icon,
      lastAt: threads.find(th => th.topic === t.id)?.updatedAt,
    }))
  );
  ...
  const navApps = $derived<NavItem[]>(
    ws.apps.map(a => {
      ...
      return {
        ...
        label: a.name,
        icon:  '🔌',
        lastAt: threads.find(th => th.topic === id)?.updatedAt,
      };
    })
  );
  ...
  onMount(async () => {
    ...
    try {
      ...
      const res     = await fetch(`${publicWebConfig.apiUrl}/threads`, {
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
      if (res.ok) threads = await res.json();
    } catch { /* threads stays empty */ }
  });
  ...
  function selectNav(id: string) {
    ...
    const thread = threads.find(t => t.topic === id);
    if (thread) {
      historyMsgs = [
        ...
        {#each navTopics as item}
          ...
          >
            ...
            {#if selectedNavId === item.id}
              <span class="ml-auto h-1.5 w-1.5 rounded-full bg-indigo-400"></span>
            {:else if item.lastAt}
              <span class="ml-auto text-[10px] text-zinc-700">{relativeTime(item.lastAt)}</span>
            {:else}
